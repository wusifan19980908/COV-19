...
public class QRCodeUtil {
    ...
    private static int FRONT_COLOR = 0x000000;
    private static final int BACKGROUND_COLOR = 0xFFFFFF;


    /**
     * @param map        二维码参数内容，如果是一个网页地址，如 https://www.baidu.com/ 则 微信扫一扫会直接进入此地址， 如果是一些参数，如
     *                           1541656080837，则微信扫一扫会直接回显这些参数值
     * @param codeImgFileSaveDir 二维码图片保存的目录,如 D:/codes
     * @param fileName           二维码图片文件名称，带格式,如 123.png
     */
    public static void createCodeToFile(Map<String,Object> map, File codeImgFileSaveDir, String fileName) {
        try {
            if (map.get("msg") == null || "".equals(map.get("msg"))) {
                log.info("二维码内容为空，不进行操作...");
                return;
            }
            String msg = (String) map.get("msg").toString().trim();
            ...
            BufferedImage bufferedImage = getBufferedImage(msg, (String) map.get("color"));
    ...
    private static BufferedImage getBufferedImage(String codeContent,String color) throws WriterException {
        /*
         ...
         */
        if ("0".equals(color)){
            FRONT_COLOR = 	0x3CB371;
        }
        if ("1".equals(color)){
            FRONT_COLOR = 	0xFFFF00;
        }
        if ("2".equals(color)){
            FRONT_COLOR = 	0xFF7F50;
        }
        if ("3".equals(color)){
